...
envsubst '${MICROSERVICE_AUTH_HOST} ${MICROSERVICE_AUTH_PORT} ${MICROSERVICE_AUTH_REALM} ${MICROSERVICE_CART_HOST} ${MICROSERVICE_CART_PORT}' < /etc/nginx/nginx.conf.template > /etc/nginx/nginx.conf
